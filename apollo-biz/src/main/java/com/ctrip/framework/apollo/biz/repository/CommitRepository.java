...
public interface CommitRepository extends PagingAndSortingRepository<Commit, Long> {
  ...
  @Modifying
  @Query(nativeQuery = true,value = "update Commit set IsDeleted = 1, DeletedAt = (EXTRACT(epoch FROM now()))::::bigint *1000, DataChange_LastModifiedBy = ?4 where appId=?1 and clusterName=?2 and namespaceName = ?3")
