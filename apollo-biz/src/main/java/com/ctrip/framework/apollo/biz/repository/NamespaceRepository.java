...
public interface NamespaceRepository extends PagingAndSortingRepository<Namespace, Long> {
  ...
  @Modifying
  @Query(nativeQuery = true,value = "update Namespace set IsDeleted = 1, DeletedAt = (EXTRACT(epoch FROM now()))::::bigint *1000, DataChange_LastModifiedBy = ?3 where appId=?1 and clusterName=?2")
