...
public interface ReleaseRepository extends PagingAndSortingRepository<Release, Long> {
  ...
  @Modifying
  @Query(nativeQuery = true, value ="update \"Release\" set \"IsDeleted\" = true, \"DeletedAt\" = ROUND(extract (epoch from now())), \"DataChange_LastModifiedBy\" = ?4 where \"AppId\"=?1 and \"ClusterName\"=?2 and \"NamespaceName\" = ?3 and \"IsDeleted\" = false")
