...
public interface ConsumerRoleRepository extends PagingAndSortingRepository<ConsumerRole, Long> {
  ...
  @Modifying
  @Query(nativeQuery = true, value ="UPDATE \"ConsumerRole\" SET \"IsDeleted\" = true, \"DeletedAt\" = ROUND(extract (epoch from now())), \"DataChange_LastModifiedBy\" = ?2 WHERE \"RoleId\" in ?1 and \"IsDeleted\" = false")
