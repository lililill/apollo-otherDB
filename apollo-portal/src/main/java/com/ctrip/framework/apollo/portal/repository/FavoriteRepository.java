...
public interface FavoriteRepository extends PagingAndSortingRepository<Favorite, Long> {
  ...
  @Modifying
  @Query(nativeQuery = true, value ="UPDATE \"Favorite\" SET \"IsDeleted\" = true, \"DeletedAt\" = ROUND(extract (epoch from now())), \"DataChange_LastModifiedBy\" = ?2 WHERE \"AppId\"=?1 and \"IsDeleted\" = false")
